...
const IdleTimer = () => {
  ...
  const startShutDown = async () => {
    ...
    await window.api.idleTimer_shutdown(totalSeconds);
 
  };
  const handleCancel = async () => {
    setIsStart(false);
    setRemainTime(null);
    const result = await window.api.cancelShutdownIdleTimer();
    if (result.status == "ok") {
      openModalCancel();
      ...
      <div className="flex justify-center items-center flex-col h-[50vh]">
        ...
        {remaingTime && (
          <div className=" text-[12px] font-bold text-yellow-500  flex items-center gap-3  my-5">
            <span>
              {remaingTime ? String(remaingTime.seconds).padStart(2, 0) : "00"}
            </span>

            <span>:</span>

            <span>
              {remaingTime ? String(remaingTime.minutes).padStart(2, 0) : "00"}
            </span>
            <span>:</span>

            <span>
              {remaingTime ? String(remaingTime.hours).padStart(2, 0) : "00"}
            </span>
            <span>زمان عدم فعالیت</span>
          </div>
        )}
